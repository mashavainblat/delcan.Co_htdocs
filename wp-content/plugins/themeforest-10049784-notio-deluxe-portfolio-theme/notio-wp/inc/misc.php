			...
			<div class="small-12 large-10 columns">
				<div class="vcenter">
					<div class="product_share">
						<h4><?php _e('Share This Work', 'notio'); ?></h4>
...
<?php
	function thb_add_searchform() { 
		...
		<aside id="searchpopup">
			<div class="spacer"></div>
			<div class="vcenter">
					<p><?php _e('SEARCH AND PRESS ENTER', 'notio'); ?></p>
					<?php get_search_form(); ?>
			</div>
		</aside>
		<?php
...
function thb_site_bars() {
	...
	?>
		...
		<aside id="bar-left" class="bar-side left-side site_bars_portfolio-<?php echo esc_attr($site_bars_portfolio); ?> <?php if ($site_bars_portfolio_position == 'left') { echo 'active'; } ?>">
			...
			<?php if ($site_bars_portfolio_position == 'left') { echo $out; } ?>
		</aside>
		<!-- End Left Bar -->
		...
		<aside id="bar-right" class="bar-side right-side site_bars_portfolio-<?php echo esc_attr($site_bars_portfolio); ?> <?php if ($site_bars_portfolio_position == 'right') { echo 'active'; } ?>">
			...
			<?php if ($site_bars_portfolio_position == 'right') { echo $out; } ?>
		</aside>
		<!-- End Right Bar -->
	<?php
	}
}
add_action( 'thb_site_bars', 'thb_site_bars' );
	...
	if (sizeof($socials) > 0 && $socials !== '') {
		foreach ($socials as $social) {
			...
			<a href="<?php echo esc_url($social['href']); ?>" class="<?php echo esc_attr($social['social_network']); ?> icon-1x" target="_blank">
				...
				<?php if ($text) { ?><span><?php echo esc_attr($social['social_network']); ?></span><?php } ?></a>
			</a><?php
		}
	}
}
					...
					<a href="<?php echo get_permalink($prev->ID); ?>" class="post_nav_link prev">
						...
						<strong>
							<?php if (is_singular('portfolio')) { ?>
								<?php esc_html_e('Previous Project (p)', 'notio'); ?>
							<?php } else if (is_singular('product')) { ?>
								<?php esc_html_e('Previous Product (p)', 'notio'); ?>
							<?php } else { ?>
								<?php esc_html_e('Previous Post (p)', 'notio'); ?>
							<?php } ?>
						</strong>
			...
			<div class="small-5 columns">
				...
				?>
					<a href="<?php echo get_permalink($next->ID); ?>" class="post_nav_link next">
						...
						<strong>
							<?php if (is_singular('portfolio')) { ?>
								<?php esc_html_e('Next Project (n)', 'notio'); ?>
							<?php } else if (is_singular('product')) { ?>
								<?php esc_html_e('Next Product (p)', 'notio'); ?>
							<?php } else { ?>
								<?php esc_html_e('Next Post (n)', 'notio'); ?>
							<?php } ?>
						</strong>
