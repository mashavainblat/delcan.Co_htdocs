		...
		<?php do_action( 'thb_product_badge'); ?> 
			<?php if ( $attachment_ids ) {								
				foreach ( $attachment_ids as $attachment_id ) {
					...
					$attr = trim($attr, ',');
					?>
						<figure class="woocommerce-product-gallery__wrapper" itemprop="image" data-variation="<?php echo esc_attr($attr); ?>">
							<?php if ($lightbox_en) { ?>
							<a href="<?php echo esc_attr($src[0]); ?>" rel="lightbox" itemprop="image" data-size="<?php echo esc_attr($src_small[1].'x'.$src_small[2]); ?>">
							<?php } ?>
								<img src="<?php echo esc_attr($src_small[0]); ?>" class="woocommerce-product-gallery__image" title="<?php echo esc_attr($image_title); ?>" />
							<?php if ($lightbox_en) { ?>
							</a>
							<?php } ?>
						</figure>
